...
def professor_input(
    ...
    *,
    ...
    return ProfessorCreate(
        ...
        title=title,
        email=email,
        directory_profile_url=f"https://ece.illinois.edu/{slug}",
        homepage_url=f"https://example.edu/{slug}",
        lab_url=f"https://example.edu/{slug}/lab",
    ...
    with repositories(tmp_path / "detail.db") as (
        ...
        proposal = proposals.create_pending(
            ProposalCreate(
                ...
                source_urls=[professor.directory_profile_url],
                confidence=0.9,
            ),
            now=NOW,
        )
